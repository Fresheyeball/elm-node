...
Elm.Http = Elm.Http || {};
Elm.Http.Server = Elm.Http.Server || {};
Elm.Http.Server.make = function (_elm) {
   "use strict";
   _elm.Http = _elm.Http || {};
   _elm.Http.Server = _elm.Http.Server || {};
   if (_elm.Http.Server.values) return _elm.Http.Server.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   var _op = {};
   var runResponse = F2(function (res,rawres) {
      var _p0 = res;
      switch (_p0.ctor)
      {case "Html": return A2($Http$Server$Raw.writeHtml,
           rawres,
           $VDOMtoHTML.toHTML(_p0._0));
         case "Json": return A2($Http$Server$Raw.writeJson,
           rawres,
           _p0._0);
         case "Text": return A2($Http$Server$Raw.writeText,
           rawres,
           _p0._0);
         default: return $Task.succeed({ctor: "_Tuple0"});}
   });
   var Request = F3(function (a,b,c) {
      return {url: a,method: b,statusCode: c};
   });
   var marshallRequest = function (req) {
      return A3(Request,
      $Http$Server$Raw.url(req),
      $Http$Server$Raw.method(req),
      $Http$Server$Raw.statusCode(req));
   };
   var run = F2(function (p,s) {
      var server = $Signal.mailbox({ctor: "_Tuple2"
                                   ,_0: $Http$Server$Raw.emptyReq
                                   ,_1: $Http$Server$Raw.emptyRes});
      var reply = A3($Signal.map2,
      function (_p1) {
      ...
      server.address,
      p,
      A2($Basics._op["++"],"Listening on ",$Basics.toString(p))),
      $Basics.always($Task.succeed({ctor: "_Tuple0"}))));
      return A2($Signal.merge,create,reply);
   });
   var EmptyRes = {ctor: "EmptyRes"};
   var Text = function (a) {    return {ctor: "Text",_0: a};};
   var Json = function (a) {    return {ctor: "Json",_0: a};};
   var Html = function (a) {    return {ctor: "Html",_0: a};};
   return _elm.Http.Server.values = {_op: _op
                                    ,Html: Html
                                    ,Json: Json
                                    ,Text: Text
                                    ,EmptyRes: EmptyRes
                                    ,Request: Request
                                    ,marshallRequest: marshallRequest
...
Elm.Server.make = function (_elm) {
   ...
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   $Http$Server = Elm.Http.Server.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   ...
   var view = function (count) {
      var times = A2($Basics._op["++"],
      $Basics.toString(count),
      _U.eq(count,1) ? " time." : " times.");
      return A2($Html.h1,
      _U.list([]),
   ...
   var route = F2(function (_p1,_p0) {
      ...
      var _p4 = _p2.url;
      switch (_p4)
      {case "/": return {ctor: "_Tuple2"
                        ,_0: count$
                        ,_1: $Http$Server.Html(view(count$))};
         case "/reset": return {ctor: "_Tuple2"
                               ,_0: 0
                               ,_1: $Http$Server.Text("Count Reset")};
         default: return {ctor: "_Tuple2"
                         ,_0: _p5
                         ,_1: $Http$Server.Text("404")};}
   });
   var server = function (sigreq) {
      return A3($Signal.foldp,
      route,
      {ctor: "_Tuple2",_0: 0,_1: $Http$Server.EmptyRes},
      sigreq);
   };
   var serve = Elm.Native.Task.make(_elm).performSignal("serve",
   A2($Http$Server.run,8080,server));
