...
Elm.Http = Elm.Http || {};
Elm.Http.Server = Elm.Http.Server || {};
Elm.Http.Server.Raw = Elm.Http.Server.Raw || {};
Elm.Http.Server.Raw.make = function (_elm) {
   "use strict";
   _elm.Http = _elm.Http || {};
   _elm.Http.Server = _elm.Http.Server || {};
   _elm.Http.Server.Raw = _elm.Http.Server.Raw || {};
   if (_elm.Http.Server.Raw.values)
   return _elm.Http.Server.Raw.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Native$Http$Server = Elm.Native.Http.Server.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var plainText = {ctor: "_Tuple2"
                   ,_0: "Content-Tyoe"
                   ,_1: "plain/text"};
   var applicationJson = {ctor: "_Tuple2"
                         ,_0: "Content-Tyoe"
                         ,_1: "application/json"};
   var textHtml = {ctor: "_Tuple2"
                  ,_0: "Content-Type"
                  ,_1: "text/html"};
   var statusCode = $Native$Http$Server.statusCode;
   var method$ = $Native$Http$Server.method;
   var url = $Native$Http$Server.url;
   var emptyRes = $Native$Http$Server.emptyRes;
   var emptyReq = $Native$Http$Server.emptyReq;
   var end = $Native$Http$Server.end;
   var write = $Native$Http$Server.write;
   var writeHead = $Native$Http$Server.writeHead;
   var writeAs = F3(function (header,res,html) {
      return A2($Task.andThen,
      A2($Task.andThen,A3(writeHead,200,header,res),write(html)),
      end);
   });
   var writeHtml = writeAs(textHtml);
   var writeJson = function (res) {
      return function (_p0) {
         return A3(writeAs,
         applicationJson,
         res,
         A2($Json$Encode.encode,0,_p0));
      };
   };
   var writeText = writeAs(plainText);
   var listen = $Native$Http$Server.listen;
   var createServer = $Native$Http$Server.createServer;
   var createServer$ = F3(function (address,port$,echo) {
      return A2($Task.andThen,
      createServer(address),
      A2(listen,port$,echo));
   });
   var on = $Native$Http$Server.on;
   var onRequest = on("request");
   var onClose = on("close");
   var onCloseReq = on("close");
   var onCloseRes = on("close");
   var onFinishRes = on("finish");
   var NOOP = {ctor: "NOOP"};
   var DELETE = {ctor: "DELETE"};
   var PUT = {ctor: "PUT"};
   var POST = {ctor: "POST"};
   var GET = {ctor: "GET"};
   var method = function (req) {
      var _p1 = method$(req);
      switch (_p1)
      {case "GET": return GET;
   ...
   var Response = {ctor: "Response"};
   var Request = {ctor: "Request"};
   var Server = {ctor: "Server"};
   return _elm.Http.Server.Raw.values = {_op: _op
...
Elm.Server = Elm.Server || {};
Elm.Server.make = function (_elm) {
   "use strict";
   _elm.Server = _elm.Server || {};
   if (_elm.Server.values) return _elm.Server.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Http$Server$Raw = Elm.Http.Server.Raw.make(_elm),
   $Json$Encode = Elm.Json.Encode.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var route = function (_p0) {
      var _p1 = _p0;
      var _p5 = _p1._1;
      var _p4 = _p1._0;
      var _p2 = $Http$Server$Raw.method(_p4);
      switch (_p2.ctor)
      {case "GET": var _p3 = $Http$Server$Raw.url(_p4);
           switch (_p3)
           {case "/": return A2($Http$Server$Raw.writeHtml,
                _p5,
                "<h1>Wowzers</h1>");
              case "/foo": return A2($Http$Server$Raw.writeHtml,
                _p5,
                "<h1>Foozle!</h1>");
              default: return A2($Http$Server$Raw.writeHtml,
                _p5,
                "<h1>404</h1>");}
         case "POST": return A2($Http$Server$Raw.writeJson,
           _p5,
           $Json$Encode.object(_U.list([{ctor: "_Tuple2"
                                        ,_0: "foo"
                                        ,_1: $Json$Encode.string("bar")}])));
         case "NOOP": return $Task.succeed({ctor: "_Tuple0"});
         default: return A2($Http$Server$Raw.writeJson,
           _p5,
           $Json$Encode.string("fail"));}
   };
   var server = $Signal.mailbox({ctor: "_Tuple2"
                                ,_0: $Http$Server$Raw.emptyReq
                                ,_1: $Http$Server$Raw.emptyRes});
   var reply = Elm.Native.Task.make(_elm).performSignal("reply",
   A2($Signal.map,route,$Signal.dropRepeats(server.signal)));
   var serve = Elm.Native.Task.make(_elm).perform(A3($Http$Server$Raw.createServer$,
   server.address,
   8080,
   "Listening on 8080"));
   return _elm.Server.values = {_op: _op
                               ,server: server
                               ,route: route};
};
Elm.worker(Elm.Server);
